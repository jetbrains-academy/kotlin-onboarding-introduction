...
```kotlin
fun main() {
  applyBordersFilter(simba) // Example with the simba picture
  applyBordersFilter(monkey) // Example with the monkey picture
  applyBordersFilter(android) // Example with the android picture (handles variable line lengths)
}
```
</div>
