...
class Test {
    ...
    fun testPrintRoundResultsFunctionImplementation(
        guess: String,
        secret: String,
        expectedPosMatchings: Int,
        expectedLettersMatchings: Int
    ) {
        val userMethod = mainClass.findMethod(mainClazz, printRoundResultsMethod)
        val baos = setSystemOut()
        val expectedOutput = "Your guess has $expectedPosMatchings full matches and $expectedLettersMatchings partial matches.$newLineSymbol"
        val errorMessage = "The method ${printRoundResultsMethod.name} should accept guess and secret arguments and print $expectedOutput into console!"
        try {
